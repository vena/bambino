...
- <span id="amsfilamentsettingrequest-with-setting-id"></span>`fn with_setting_id(self, setting_id: &str) -> Self`
  ...
  Follows the `with_*` convention [`PrintJobConfig`](commands/print_job/index.md#printjobconfig) already uses,
  rather than a tenth positional argument on [`new`](commands/ams/index.md#amsfilamentsettingrequest).

  Pass the long form here — `"GFSL05_07"`, or a `"PF"`-prefixed id — and keep the short
  code in `tray_info_idx`. See [`AmsFilamentSettingPayload::tray_info_idx`](commands/ams/index.md#amsfilamentsettingpayload) for what the
