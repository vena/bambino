...
- `TimerFailure(crate::io::TimerError)` - Encapsulates platform timer/sleep scheduling failures (e.g. ESP-IDF FreeRTOS timer resource exhaustion).
...
- `ModelMismatch(std::borrow::Cow<'static, str>)` - Emitted when requesting capabilities (e.g. door sensor checking on an open-frame printer) not present on the active model target.
