...
- [Types](#types)
  - [`CnFallbackServerVerifier`](#cnfallbackserververifier)
  - [`NoCertificateVerification`](#nocertificateverification)
  - [`TokioIo`](#tokioio)
...
|------|------|-------------|
| [`CnFallbackServerVerifier`](#cnfallbackserververifier) | struct | Certificate verifier for the "verified" (CA-checked) connection path that validates real chain-of-trust against caller-supplied trusted roots, but — unlike rustls's default `WebPkiServerVerifier` — works against real Bambu printer certs at all. |
| [`NoCertificateVerification`](#nocertificateverification) | struct | Custom certificate verifier that bypasses standard CA chain authority validation. |
