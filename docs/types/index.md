*[bambino](../index.md) / [types](index.md)*
...
Shared data types used across the crate — most importantly [`PrinterTelemetry`](telemetry/report/index.md#printertelemetry),
the deserialized form of the JSON state reports the printer pushes over MQTT.
Also includes [`VersionInfo`](version/index.md#versioninfo) for firmware version queries and AMS/device
sub-structures like [`AmsTray`](telemetry/ams/index.md#amstray), [`DeviceTelemetry`](telemetry/device/index.md#devicetelemetry), and [`ExtruderInfo`](telemetry/device/index.md#extruderinfo).
...
- <span id="airductmodelistentry-clone"></span>`fn clone(&self) -> AirductModeListEntry` — [`AirductModeListEntry`](telemetry/device/index.md#airductmodelistentry)
...
- <span id="airductpart-clone"></span>`fn clone(&self) -> AirductPart` — [`AirductPart`](telemetry/device/index.md#airductpart)
...
- <span id="amsdrysetting-clone"></span>`fn clone(&self) -> AmsDrySetting` — [`AmsDrySetting`](telemetry/ams/index.md#amsdrysetting)
...
- <span id="amsstatusreport-clone"></span>`fn clone(&self) -> AmsStatusReport` — [`AmsStatusReport`](telemetry/ams/index.md#amsstatusreport)
...
- <span id="amstray-default"></span>`fn default() -> AmsTray` — [`AmsTray`](telemetry/ams/index.md#amstray)
...
- <span id="bedinfo-clone"></span>`fn clone(&self) -> BedInfo` — [`BedInfo`](telemetry/device/index.md#bedinfo)
...
- <span id="bedtelemetry-clone"></span>`fn clone(&self) -> BedTelemetry` — [`BedTelemetry`](telemetry/device/index.md#bedtelemetry)
...
- <span id="ctctelemetry-clone"></span>`fn clone(&self) -> CtcTelemetry` — [`CtcTelemetry`](telemetry/diagnostics/index.md#ctctelemetry)
...
- <span id="extrudercollection-clone"></span>`fn clone(&self) -> ExtruderCollection` — [`ExtruderCollection`](telemetry/device/index.md#extrudercollection)
...
- <span id="hmsentry-clone"></span>`fn clone(&self) -> HmsEntry` — [`HmsEntry`](telemetry/diagnostics/index.md#hmsentry)
...
- <span id="netinfo-clone"></span>`fn clone(&self) -> NetInfo` — [`NetInfo`](telemetry/report/index.md#netinfo)
...
- <span id="nozzlecollection-clone"></span>`fn clone(&self) -> NozzleCollection` — [`NozzleCollection`](telemetry/device/index.md#nozzlecollection)
...
- **`diameter`**: `Option<f32>`
  ...
  **Can be stale.** An empty hotend is still reported in `nozzle_info`, carrying the
  ...
  presence with [`NozzleInfo::is_installed()`](telemetry/device/index.md#nozzleinfo) before trusting this.
...
- **`stat`**: `Option<u32>`
  ...
  **Not a presence indicator.** It read `0` on every entry of an H2C's `nozzle_info`,
  occupied and empty alike — use [`NozzleInfo::is_installed()`](telemetry/device/index.md#nozzleinfo) instead.
...
- <span id="nozzleinfo-is-installed"></span>`fn is_installed(&self) -> bool`
  ...
  the [`diameter`](telemetry/device/index.md#nozzleinfo) of whatever it last held. Measured on an idle H2C, an
  ...
  * the temperature rating must be absent or zero.
  ...
  [`stat`](telemetry/device/index.md#nozzleinfo) is not usable for this — it read `0` on every entry, occupied and
  ...
  [`is_rack_stored()`](telemetry/device/index.md#nozzleinfo)).
...
- **`xcam`**: `Option<super::xcam::XcamTelemetry>`
  ...
  carries it. Prefer [`xcam`](telemetry/xcam/index.md), which caches and merges it.
...
- **`stg`**: `Option<Vec<i32>>`
  ...
  follow BambuStudio's `get_stage_string` table; decode them with [`Self::stage_queue`](telemetry/report/index.md#printertelemetry).
...
- <span id="telemetryreport-clone"></span>`fn clone(&self) -> TelemetryReport` — [`TelemetryReport`](telemetry/index.md#telemetryreport)
...
- <span id="xcamdetector-clone"></span>`fn clone(&self) -> XcamDetector` — [`XcamDetector`](telemetry/xcam/index.md#xcamdetector)
...
- <span id="xcamdetector-partialeq-eq"></span>`fn eq(&self, other: &XcamDetector) -> bool` — [`XcamDetector`](telemetry/xcam/index.md#xcamdetector)
...
none of it. Use [`merge_from`](telemetry/xcam/index.md#xcamtelemetry) rather than replacing a cached copy wholesale.

Unmodeled keys survive in [`extra`](telemetry/xcam/index.md#xcamtelemetry): this wire object is still largely uncharted
...
- **`cfg`**: `Option<u32>`
  ...
  at all — see [`supports_ai_monitoring`](telemetry/xcam/index.md#xcamtelemetry).
...
- <span id="xcamtelemetry-spaghetti-detection"></span>`fn spaghetti_detection(&self) -> Option<XcamDetector>` — [`XcamDetector`](telemetry/xcam/index.md#xcamdetector)

  Returns spaghetti-detection state decoded from `cfg`, or `None` when `cfg` is absent.

- <span id="xcamtelemetry-purge-chute-pileup-detection"></span>`fn purge_chute_pileup_detection(&self) -> Option<XcamDetector>` — [`XcamDetector`](telemetry/xcam/index.md#xcamdetector)

  Returns purge-chute-pileup detection state decoded from `cfg`, or `None` when `cfg` is absent.

- <span id="xcamtelemetry-nozzle-clumping-detection"></span>`fn nozzle_clumping_detection(&self) -> Option<XcamDetector>` — [`XcamDetector`](telemetry/xcam/index.md#xcamdetector)

  Returns nozzle-clumping detection state decoded from `cfg`, or `None` when `cfg` is absent.

- <span id="xcamtelemetry-air-printing-detection"></span>`fn air_printing_detection(&self) -> Option<XcamDetector>` — [`XcamDetector`](telemetry/xcam/index.md#xcamdetector)
...
- <span id="xcamtelemetry-clone"></span>`fn clone(&self) -> XcamTelemetry` — [`XcamTelemetry`](telemetry/xcam/index.md#xcamtelemetry)
...
- <span id="xcamtelemetry-default"></span>`fn default() -> XcamTelemetry` — [`XcamTelemetry`](telemetry/xcam/index.md#xcamtelemetry)
...
- <span id="versioninfo-clone"></span>`fn clone(&self) -> VersionInfo` — [`VersionInfo`](version/index.md#versioninfo)
...
- <span id="versionmodule-clone"></span>`fn clone(&self) -> VersionModule` — [`VersionModule`](version/index.md#versionmodule)
...
- <span id="amsfilamentstep-partialeq-eq"></span>`fn eq(&self, other: &AmsFilamentStep) -> bool` — [`AmsFilamentStep`](telemetry/ams/index.md#amsfilamentstep)
...
- <span id="xcamsensitivity-clone"></span>`fn clone(&self) -> XcamSensitivity` — [`XcamSensitivity`](telemetry/xcam/index.md#xcamsensitivity)
...
- <span id="xcamsensitivity-partialeq-eq"></span>`fn eq(&self, other: &XcamSensitivity) -> bool` — [`XcamSensitivity`](telemetry/xcam/index.md#xcamsensitivity)
