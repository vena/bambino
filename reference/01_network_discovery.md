...
The printer's X.509 certificates contain the printer's unique uppercase serial number in the Common Name (CN) field. The leaf is **not** self-signed: it is issued by Bambu's private `BBL CA` root, and a live P1S (firmware 01.10.00.00) has been verified end-to-end against those anchors — a real chain of trust exists, so `--with-certs` is a working verification path rather than a diagnostic-only one. The leaf itself is X.509 **v1** with no SAN. During TLS handshakes, the printer expects the Server Name Indication (SNI) extension to match this serial number if hostname verification is enforced by the connecting peer.

#### X.509 Certificate Key Usage Constraints
The certificates generated by the physical printer's embedded server do not contain X.509 v3 "Key Usage" extension blocks.
