...
3. **Staleness vs. current code.** Do the code examples, method signatures, and described behavior still match what's actually in `src/`? Check especially against any GitHub Issues closed since the last README review — a fix that changed public API shape or documented behavior is the exact class of change that makes an example silently wrong. Don't assume; grep the actual current signature for anything README shows a code sample for.
