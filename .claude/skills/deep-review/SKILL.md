---
name: deep-review
description: Runs a parallel, module-by-module deep code review sweep across the whole bambino crate — discovers the current src/ and tests/ structure, spawns one review agent per module boundary, and compiles findings into a dated review file. Confident bugs get filed as new GitHub Issues immediately; plausible-but-unverified findings are re-verified before the sweep finishes and either filed as an issue (confirmed) or noted inline as not-a-bug (never filed) — not left for manual triage. Use when asked for a "full review", "deep review", "review sweep", to audit the whole crate/codebase, or to check for accumulated bugs before a release milestone. Not for reviewing a single diff/PR/recent change — use the code-review skill for that instead.
---
...
1. **Dedupe** — `gh issue list --search "<file/topic keyword>" --state all --limit 100` to check for an existing open or closed issue on the same file/topic before treating a finding as new (a finding that resurfaces from a prior sweep is a regression, not a new bug — note that distinction in the review file rather than silently double-filing).
2. **Assign severity** to `CONFIRMED` findings only, per the `backlog` skill's rubric (sev1/sev2/sev3/needs-verification) — don't re-derive or duplicate those definitions here, invoke that skill's rules directly. `PLAUSIBLE` findings don't get a severity yet; that happens if/when a human triages one into a real issue later.
3. **File `CONFIRMED` findings as a GitHub Issue now**, using only the `backlog` skill's Issue format and Severity section — don't wait until every unit finishes. Nothing else from that skill applies mid-sweep: not Docs regen, not the "what counts as an issue" non-bug case (a `CONFIRMED` finding is by definition a real bug) — those are separate, human-triggered concerns, out of scope for this step.
4. **Replace that unit's `PENDING` placeholder** in `MM-DD-REVIEW.md` with its real content: `CONFIRMED` findings in Step 3's Issue/Detail/Suggested-fix format, each linked to its new issue number; or `NO ISSUES FOUND in <unit>` if it came back clean of both tiers. Append any `PLAUSIBLE` findings to a `## Plausible, Unverified Findings` section (create it on the first one) — these don't get an issue yet, flag the section for the user to manually triage. Update the `Status` line's count.
...
Once every unit's `PENDING` placeholder is gone: re-verify every `PLAUSIBLE` finding by direct code read (not by re-stating the agent's claim), then either file it as a GitHub Issue (confirmed real, with severity, per the `backlog` skill's rules) or note it inline in the review file as triaged-not-a-bug (never filed — see that skill's "What counts as an issue") — don't leave any sitting in the review file for a human to ask about separately; that manual-triage gap is exactly what prior sweeps under this skill required a follow-up prompt for. Then write the closing summary table (`Issue # | Sev | Module | File(s) | One-line`) mapping every filed finding to its issue number and severity, with a note directly above it that GitHub Issues is the status source of truth from here on — the table itself is a point-in-time snapshot and won't be updated as bugs get fixed. Flip the top-of-file `Status` line to `**Status:** COMPLETE`. Don't rely on a prior sweep's review file surviving as a template for any of this — a fully-resolved one is deleted in its own signposted commit, same convention as a completed `*_PLAN.md` (see `CLAUDE.md`'s Key Conventions); `git log --all -- '*-REVIEW.md'` finds a deleted one's content if a concrete example is wanted.

## Step 6 — Report

Discovery + issue filing (including Step 5's `PLAUSIBLE` triage) is the whole job — even an obviously-trivial fix found along the way isn't yours to make in this sweep.

Summarize to the user: unit count, clean vs. flagged, new issue numbers and their severities (including any from Step 5's triage), and point at the two artifacts (`MM-DD-REVIEW.md`, the filed GitHub Issues) rather than dumping full findings inline into the conversation. Ask whether to commit the review file now — don't commit without that confirmation, per this project's standing git-safety rule. (Issue filing itself isn't a git commit, so it isn't gated the same way — but confirm before filing a large batch in one go if the sweep surfaced many findings, same spirit as the git-safety rule.)
