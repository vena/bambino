...
5. **Scope rules** (bambino-specific policy, keep these unless the project's stated design changes):
   ...
   - Skip style/refactor/abstraction suggestions entirely, and skip naming preferences — except where a name actively misrepresents behavior (e.g. implies the opposite of what the function does, or a boolean whose sense is inverted from what its name suggests). That's a correctness/footgun risk wearing a naming-shaped disguise, not a style preference — same class as a doc comment that says one thing while the code does another.
