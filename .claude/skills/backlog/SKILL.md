...
`ctx_search(pattern="BUG-[0-9]+", path="BACKLOG.md")` across `Open` + `Fixed` + `Wontfix` — IDs only, no need to read row prose — take the highest `BUG-NNN`, use `NNN+1`. Never reuse a retired number, never restart numbering because a section looks short.
