...
This file is rules, not a self-driving procedure — unlike `deep-review`, it doesn't discover its own task. Invoked bare (no task given, e.g. a standalone `/backlog` with no further instruction): read `BACKLOG.md` in full first (it's data-only, small, safe to read wholesale) before asking the user what to do — don't guess a task from silence. Invoked to add a new bug (no `BUG-ID` yet): rule 3 bars a duplicate row for a bug already tracked elsewhere, so before adding, `ctx_search` the new bug's file/topic keyword across all three sections (`Open`/`Fixed`/`Wontfix`) with `max_results` set well above the file's current row count (e.g. 300) to rule that out — `ctx_search` caps results by default (verified empirically: 20 of 126 real matches on this file with no truncation notice), so an uncapped call is required for the dedupe to actually be exhaustive, not just cheaper. If the result count comes back at or near the `max_results` ceiling, that's a signal of possible truncation — fall back to a full read rather than trust a maybe-incomplete set. Invoked with a task naming a specific existing `BUG-ID` (close/triage/reassign severity): the row schema is fully specified below in this file already, so a full-file read isn't needed to know how to format the edit — `ctx_search` for that `BUG-ID` to get its row (and confirm which section it's currently in), read that row plus its dated `NN-NN-REVIEW.md` section (if `Detail` links one) — no need to load every other row along with it. Invoked to check the release bar: only the `Open` section's `Sev` column matters — search/read `Open` alone, skip `Fixed`/`Wontfix`. Invoked with "fix everything in `Open`": read the `Open` section in full (every row is in scope), still no need for `Fixed`/`Wontfix` — rule 6's one-commit-per-bug atomicity makes this resumable for free regardless (interrupted partway, whatever's still in `Open` is what's left). Invoked by `deep-review` mid-sweep: that skill already supplies the specific finding and file context and already dedupes against `BACKLOG.md` itself before handing off — no separate read needed here. Work severity order (Sev1, Sev2, Sev3, per the release bar below) so an interruption leaves the least release-blocking bugs behind, not the reverse — and re-locate each bug by its Title before editing, not by a possibly-stale `file:line` from `Detail`, since an earlier fix in the same sitting can shift lines a later bug also touches. Re-check Step 0 before starting each bug in the sequence, not just once at invocation — a single statement made at the start of a long loop decays by bug 5 or 6; restating it per-iteration doesn't (skip it only if that check already failed earlier in this same run — don't retry a dead MCP server every iteration).
...
`ctx_shell("grep -oE 'BUG-[0-9]+' BACKLOG.md | sort -t- -k2 -n | tail -1")` across `Open` + `Fixed` + `Wontfix` — bare IDs only, sorted numerically, exhaustive by construction (unlike `ctx_search`, which caps results by default and returns full row prose per match, not bare IDs — verified empirically: it truncated to 20 of 126 real matches on this file). Take the highest `BUG-NNN`, use `NNN+1`. Never reuse a retired number, never restart numbering because a section looks short.
