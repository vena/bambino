...
- **Global tray id is `24..=27`** (`AMS_LITE_ON_A2L_GLOBAL_TRAY_IDS`), BambuStudio's `AMS_LITE_MIXED_TRAY_INDEX_OFFSET + slot` (`DevFilaSystem.cpp:262-263`), used for `extrusion_cali_sel`'s `tray_id`. bambuddy's `64..=67` is its own unconfirmed extrapolation — don't adopt it.
