...
`race`, `Raced`, and `read_chunk` live in `src/io/mod.rs` (`pub(crate)`) — shared by `mqtt/client` and `camera/binary.rs`. `read_exact_packet`/`FrameReadState` live in `src/mqtt/client/frame.rs` (MQTT-specific). `read_next_frame()` delegates to `read_next_frame_with_timer(&DummyTimer)` — don't give it an independent implementation; both share one `read_state` field. `ftps/protocol.rs` mirrors the same stall-timeout shape for the FTPS data channel, and satisfies the partial-progress half of the hinge with its persistent control-channel `fill_buf` rather than a `FrameReadState`: `read_line_raw` never moves bytes out of `fill_buf` into its per-call `line_buf` until a complete line is buffered, so a deadline firing mid-line leaves the partial line where the next call finds it. Don't reintroduce an eager drain into `line_buf` — the safety would then rest only on `.claude/rules/ftps-poisoning.md`'s never-un-poison convention.
