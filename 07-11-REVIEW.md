...
9. ~~`src/discovery/parser.rs:182,186` — `httparse::parse(buf).ok()?` discards the `Status::Partial` vs `Status::Complete` distinction, so a truncated SSDP packet can still parse successfully.~~ **PROMOTED to BUG-086 (Sev3).** Now rejects unless `Status::Complete`.
10. ~~`src/discovery/parser.rs:178` — `is_response`'s status-line check only recognizes exact-case `"HTTP/"`/`"http/"`, inconsistent with this file's otherwise-thorough case-insensitive header handling.~~ **PROMOTED to BUG-085 (Sev3).** Classification made case-insensitive via `eq_ignore_ascii_case` — `httparse::Response::parse` itself still requires exact-case, so real-world behavior is unchanged (no firmware observed emitting non-canonical case), but this is now consistent with the file's other case-insensitive handling.
11. ~~`src/discovery/parser.rs:68` — `parse_location`'s port parsing silently coerces any unparseable port string to `80`, indistinguishable from "no port specified."~~ **PROMOTED to BUG-084 (Sev3).** Unparseable port now rejects the packet instead of defaulting.

**Unit 11 — src/ftps/ (client, mod)**
12. `src/ftps/client.rs:521` — `list_directory`'s final-reply check rejects `426` unconditionally, unlike `upload_file`/`download_file`. **Flagged as needs-verification (BUG-089)** — `reference/02_ftps.md` §2.3 documents the TCP-close-vs-`226` race as general, not P2S/X2D-specific, but a `LIST` payload has no `SIZE`-equivalent integrity recheck; genuinely needs a real wire capture, not a guess.
13. `src/ftps/client.rs` — README's "safe despite fail-open" justification doesn't cover `list_directory`. **Flagged as needs-verification (BUG-089)**, tracked together with #12 — same open question.
...
15. ~~`src/ftps/protocol.rs:360,239-345` — control-character boundary bytes (`0x20`/`0x7F`) and repeated raw reply-code offsets (`3`/`4`) aren't extracted to named `pub(crate) const`s, unlike the rest of this file's literals.~~ **PROMOTED to BUG-087 (Sev3).** Extracted to `FTP_REPLY_CODE_LEN`/`FTP_REPLY_SEPARATOR_OFFSET`/`FTP_REPLY_TEXT_OFFSET`/`FTP_PATH_CONTROL_CHAR_MAX`/`FTP_PATH_DEL_CHAR`, pure refactor.
16. ~~`src/ftps/parser.rs:118-126` — filename reconstruction via `name_tokens.join(" ")` collapses multiple consecutive spaces in a real filename to one, contradicting the struct doc's "preserving single spaces" claim.~~ **PROMOTED to BUG-088 (Sev2).** Confirmed on real P1S hardware via `bambino-cli` (upload/list/delete round-trip with a 3-space filename) that this desyncs the reported name from the actual on-disk name and silently breaks `delete_file`. Fixed by preserving the exact original substring instead of re-tokenizing/rejoining.
