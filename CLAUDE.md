...
- **`PrinterClient::poll_telemetry()`** returns `TelemetryEvent` (discriminated enum), not raw `MqttMessage`. Use `poll_raw()` or `BambuMqttClient::poll_telemetry()` for raw access. Both drain the internal `pending_messages` buffer before reading from the wire — command-response methods like `get_version()` buffer non-matching messages there.
- **`PrinterClient` constructors:** `new(connector, ip, serial, access_code, model)` creates a lazy client — MQTT connects on first use via `ensure_mqtt()`. `from_mqtt(mqtt_client, serial, model)` wraps a pre-connected `BambuMqttClient` (tests, Embassy). Both default to `DummyTimer` (message-count safety valve only). Chain `.with_timer(timer)` for wall-clock timeouts via `TimerProvider::now_millis()`. `DummyTimer::now_millis()` returns 0, so the elapsed-time check in `poll_until` never fires. Use `connect_mqtt()` for eager connection, `mqtt_connected()` for status.
