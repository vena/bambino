...
All network I/O goes through abstract traits: `AsyncIo`, `TlsConnector`, `AsyncUdpSocket`, `TimerProvider`. Platform-specific implementations live in `tokio.rs`, `esp_idf.rs`, `embassy.rs`. Never use `tokio::` or `std::net::` directly in library code outside `src/io/`. `TimerProvider::sleep` takes `&self` (instance method) to support platform timers that carry state (e.g., ESP-IDF's `EspAsyncTimer`). `EmbassyTlsConnector` is generic over `Rng: CryptoRng + RngCore` — callers must inject a hardware RNG. Static TLS buffers on embassy are guarded by an `AtomicBool` + RAII `BufferGuard` enforcing single-connection exclusivity.

### Quirks Engine (`src/quirks/`)
Printer-model differences (TLS modes, fan rounding, Z-axis safety, door sensors, camera timestamps) are handled polymorphically via the `ModelQuirks` trait. Each model family has strategy structs in `src/quirks/models/` (a1.rs, p1.rs, p2.rs, x1.rs, x2.rs, h2.rs). Models that need differentiation within a family use separate structs in the same file (e.g., `X1CQuirks`/`X1EQuirks` in x1.rs, `H2SQuirks`/`H2DQuirks`/`H2DProQuirks`/`H2CQuirks` in h2.rs). All model-specific behavior modifications must go through the quirks engine. Access quirks via `model.quirks()` — never match on `BambuModel` variants for behavioral dispatch elsewhere.

The trait provides default implementations for `is_unsupported_command()` (returns false), `is_unsafe_homing_command()` (G28+axis check for bed-on-Z models), `relative_z_move_gcode()` (parameterized via `z_max()`), and new capability methods (`requires_wallclock_rtsp_timestamps()`, `supports_auxiliary_right_fan()`, `auxiliary_fan_uses_percentage()`). The `format_z_move_gcode()` helper in `quirks/mod.rs` generates safe Z-move G-code with bounds validation. Door sensor reading uses `PrintTelemetry::is_door_open_from_home_flag()` (X1 series) or `is_door_open_from_stat()` (H2/P2/X2 series).
