...
- Public API changes need `docs/` regenerated via `make docs` (see the `docs` Makefile target's own comment for why it's not hooked per-commit). The `backlog` skill's review-file-lifecycle step covers this for batch bug-fix work; for anything else, run it yourself before considering the change done.
