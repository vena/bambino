...
- **`PrinterClient` timer contract:** `new()` uses `DummyTimer` (no wall-clock timeout — only message-count safety valve). Use `new_with_timer()` for integrated command-response timeouts via `TimerProvider::now_millis()`. `DummyTimer::now_millis()` returns 0, so the elapsed-time check in `poll_until` never fires.
