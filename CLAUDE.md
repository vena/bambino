...
- **`DeviceTelemetry`** appears at two wire locations: top-level `{"device": {...}}` for incremental updates, and nested inside `{"print": {"device": {...}}}` for pushall on H2/P2/X2 models. Check both, don't depend on model.
