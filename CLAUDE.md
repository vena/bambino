...
Run `make check-fast`/`make check-all`/`git commit` through `ctx_shell`, not `Bash`, when lean-ctx is connected — their output (multi-target cargo build/test/clippy, ESP-IDF check) is large and repetitive, and `git commit` isn't in the global lean-ctx bypass list since it fires the pre-commit hook running the same output.
